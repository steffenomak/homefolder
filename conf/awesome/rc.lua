...
memicondark = wibox.widget.background(memicon, "#1A1A1A")
...
memwidgetbg = wibox.widget.background(memwidget, "#313131")

-- MEM SWAP
swapwidget = lain.widgets.mem({
    settings = function()
        widget:set_text(" " .. mem_now.swapused .. "MB ")
    end
})

swapwidgetbg = wibox.widget.background(swapwidget, "#1A1A1A")
...
for s = 1, screen.count() do
    ...
    right_layout:add(arrl_dl)
    right_layout:add(memicondark)
    right_layout:add(swapwidgetbg)
